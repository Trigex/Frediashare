...
if ($_REQUEST[goact]!="" && $err=="")
{
	...
	foreach($_REQUEST['mid'] as $value) { $conn->execute("update pmessages set outbox_track='1' where mid='".filter_title($value)."' and sender='$_SESSION[USERNAME]'"); }
    }
...
{
    $mid=filter_title($_REQUEST[mid]);
    $sql="update pmessages set outbox_track='1' where mid='$mid' and sender='$_SESSION[USERNAME]'";
    $res=mysql_query($sql);
...
// paging
$listing_per_page = $config[paging_outbox];
if($_REQUEST[page]=="")
$page = 1;
else
$page = $_REQUEST[page];
$sql = "SELECT count(*) as total from pmessages where sender='$_SESSION[USERNAME]' and outbox_track='2' $tq";
...
for($i=1; $i<=$tpage; $i++)
{
    ...
    else $page_no .= " <a href='$config[base_url]/outbox$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
}
if($page_no!="") $link = "$lang[gen_page] $page_no $lang[gen_pageof]";
//else $link = "<br><br><br><center>There are no messages!</center><br>";

if($tpage>1)
{
    $nextpage=$page+1;
    $prevpage=$page-1;
    $prevlink="<a href='$config[base_url]/outbox$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>";
    $nextlink="<a href='$config[base_url]/outbox$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
...
$sql="SELECT * from pmessages where sender='$_SESSION[USERNAME]' and outbox_track='2' $tq order by date desc limit $startfrom, $listing_per_page";
$rs = $conn->Execute($sql);
...
set_btn("messages"); set_sbtn("outbox"); set_title($lang['title_outboxmain']);
$smarty->assign('err',$err);
$smarty->assign('msg',$msg);
$smarty->display('main_header.tpl');
$smarty->display('message_outbox.tpl');
