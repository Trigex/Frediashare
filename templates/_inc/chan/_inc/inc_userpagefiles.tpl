						{if $smarty.section.v.index mod 3 eq "0" and $smarty.section.v.index gt "0"}</tr><tr>{/if}
						{if $enable_videos eq "1" and $thumb_module eq "1" and $smarty.get.view ne "audios" and $smarty.get.view ne "images" and $smarty.get.view ne "video_blog" and $smarty.get.view ne "image_blog" and $smarty.get.view ne "audio_blog"}
                                                    <script type="text/javascript">
                                                        ...
                                                        lturl['l{$rnd}']=0; ltimg['l{$rnd}']=new Array(); lthumbs['l{$rnd}']=new Array({$larra}); timg['{$rnd}']=new Array(); thumbs['{$rnd}']=new Array({$larra});
                                                    </script>
                                                {/if}
						<td align="center" class="p5" valign="top">
						    <table cellpadding="1" cellspacing="0" border="0">
							<tr>
							    <td valign=top align="center">
								{if $enable_videos eq "1"}{insert name=titlev assign=title vkey=$varr[v]}
								{elseif $enable_images eq "1"}{insert name=titlei assign=title vkey=$varr[v]}
								{elseif $enable_audio eq "1"}{insert name=titlea assign=title vkey=$varr[v]}
								{/if}
								...
								{insert name=vid_to_rndv assign=rnd vid=$vvid}
				                                ...
				                                {if $thumb_module eq "1" and $enable_videos eq "1"}
							...
							<tr>
								...
								<table width="100%" cellpadding="0" cellspacing="0">
								    <tr>
									{if $file_ratings eq "1"}<td align="left">{if $enable_videos eq "1"}{insert name=key_to_rate assign=rate vkey=$varr[v]}{elseif $enable_images eq "1"}{insert name=key_to_rate_image assign=rate vkey=$varr[v]}{elseif $enable_audio eq "1"}{insert name=key_to_rate_audio assign=rate vkey=$varr[v]}{/if}{if $vrate gt 0}{$rate}{else}<span class="bodylabel f11">{$ch_norate}</span>{/if}</td>{/if}
									<td align="{if $file_ratings eq "1"}right{else}left{/if}" {if $file_ratings eq "0"}class="nopad"{/if}>
									    ...
									    {/if}
									    {if $enable_videos eq "1" or ( $enable_videos eq "0" and $enable_images eq "0")}<span class="bodylabel f11">{$minutes}{$fileduration_min}{$seconds}{$fileduration_sec}</span>{/if}
									</td>
								    </tr>
								</table>
							    </td>
