<!-- BEGIN RELATED AUDIOS, ETC. TABLE -->
...
{insert name=vid_to_rnda assign=rnd vid=$vid[x].vid}
{insert name=vid_to_rndvv assign=rndv vid=$vid[x].vid}
{insert name=titlea assign=title vkey=$vid[x].vkey}
{if $let eq "l"}{assign var=v1 value="audio"}{assign var=v2 value="a"}{assign var=v3 value="grid"}{assign var=v4 value="audio"}{assign var=v5 value="audio"}{else}{assign var=v1 value="user"}{assign var=v2 value="u"}{assign var=v3 value="user"}{assign var=v4 value="user"}{assign var=v5 value="audio"}{/if}
{if $smarty.section.x.index mod $vpage_ftabslist eq "0" and $smarty.section.x.index gt "0"}</tr><tr>{/if}
			...
			<table cellpadding=1 cellspacing=0 border=0 width="100%">
				...
				{else}
				    <a href="{$base_url}/audio/{$vid[x].vkey}">
				{/if}
					<img src="{$tmb_url}/user{$vid[x].uid}/{$rndv}_{$rnd}.jpg" alt="{$vid[x].vtitle}" title="{$vid[x].vtitle}" width="{if $vpage_ftabslist eq "1"}110{else}90{/if}" height="70" class="{if $vid[x].vtype eq "private"}thumbp1{elseif $vid[x].is_featured eq 'yes'}thumbf1{else}thumb1{/if}">
				    </a>
				</div>
				</td>
				
				<td valign=top class="pl10">
				{insert name=key_to_rate_audio assign=rate vkey=$vid[x].vkey}
				    <table cellpadding=0 cellspacing=0 border=0>
					<tr>
					    <td><span class="title"><a href="{$base_url}/audio/{if $same_title_uploads eq "0"}{$title}{else}{$vid[x].vkey}{/if}" title="{$vid[x].vtitle}"><span class="bold">{$vid[x].vtitle|truncate:$tr_titlegrid:"..."}</span></a></td>
					</tr>
					<tr>
					    <td>
					...
					<tr>
					    <td>{$fileaddedby}{insert name=key_to_user_audio assign=user vkey=$vid[x].vkey}<a href="{$base_url}/profile/{if $special_characters eq "0"}{$user}{else}{$vid[x].uid}{/if}">{$user}</a></td>
					</tr>
					{if $smarty.request.md ne ""}
					<tr>
					    <td>{insert name=nappcount2 assign=napp2 vid=$vid[x].vid type=audio}<span class="gr">{$filecomments} </span>{$vid[x].comments-$napp2}</td>
					</tr>
