...
<table cellspacing="0" cellpadding="2"  width="950" border=0 align=center>
    <tr>
	<td class=""><h1>{$adm_play_alogoheading}</h1></td>
	...
	<td class="grey" colspan=2>
		...
		<tr>
			...
			<table width="100%" cellpadding=0 cellspacing=0>
				...
				<td width="10%" align="left">
				    <input type="button" name="wadd" class="fb" value="{$adm_play_addbtn}" onclick="location.href='{$admin_url}/watermark_audio/add'; return false;">
				</td>
			    </tr>
			</table><br>
		    </td>
		...
		<tr class="td">
		    ...
		    <td>{if $watermark[i].active eq 1}{$adm_status_active}{else}{$adm_status_inactive}{/if}</td>
    		    <td>
		    {if $watermark[i].active eq 0} <a href='{$admin_url}/watermark_audio/enable/{$watermark[i].wid}'>{$adm_play_vlogoaction1}</a>{/if}
		    {if $watermark[i].active eq 1} <a href='{$admin_url}/watermark_audio/disable/{$watermark[i].wid}'>{$adm_play_vlogoaction2}</a>{/if}{$myfiles_delim}
		    <a href="{$admin_url}/watermark_audio/edit/{$watermark[i].wid}">{$adm_play_vlogoaction3}</a>{$myfiles_delim}
    		    <a href="{$admin_url}/watermark_audio/delete/{$watermark[i].wid}" onClick='Javascript:return confirm("{$adm_play_delmsg}");'>{$adm_play_vlogoaction4}</a>
    		    </td>
		</tr>
		{/section}
		<tr>
