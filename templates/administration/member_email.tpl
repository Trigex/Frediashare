...
<table width="950" border="0" align="center" cellpadding="2" cellspacing="0">
    <tr>
	<td class=""><h1>{$adm_mememailheading}{if $to eq "all"}{$adm_mememailheadingall}{else}{$to}{/if}</h1></td>
    </tr>
</table>
<table width="950" border="0" align="center" cellpadding="3" cellspacing="0" class="br1">
	...
	<td valign="top" class="grey" align=center>
	    ...
	    <form method="post" action="{$admin_url}/members/email/{$smarty.request.to}" encType="multipart/form-data">
		<tr>
		    <td class="types">{$adm_memsendto}</td>
		    <td align="left">
		    {if $smarty.session.mid ne ""}{assign var=memsel value=$smarty.session.mid}{/if}
