...
<table width="950" cellspacing="0" cellpadding="2" border="0" align="center">
    <tr>
	<td class=""><h1>{$adm_vadsheading2}</h1></td>
	<td class="" align=right>{if $total ne "0"}{$adm_vadsnr} [{$start_num}-{$end_num}]{$adm_vadsof}[{$grandtotal}]{/if}</td>
    </tr>
</table>
<table width="950" border="0" align="center" cellpadding="5" cellspacing="0" class="br1">
	...
	<td colspan=2 class="grey">
		...
		<tr class="tablerow">
			...
			<table width="100%" cellpadding=0 cellspacing=0 border=0>
				...
				<td align="left" width="10%">
				    <input type="button" name="wadd" class="fb" value="{$adm_vadsaddbtn}" onclick="location.href='{$admin_url}/textads/add'; return false;">
				</td>
			    </tr>
			</table><br>
		    </td>
		...
		<tr class="td">
		    ...
		    <td>{$adsview[aa].descrip|truncate:20:"..."}</td>
		    ...
		    <td align="center">{$ratio[aa]}%</td>
    		    <td align="center">
		    {if $adsview[aa].active eq 0} <a href="{$admin_url}/textads/enable/{$adsview[aa].aid}">{$adm_vadsaction1}</a> {else}<a href="{$admin_url}/textads/disable/{$adsview[aa].aid}">{$adm_vadsaction2}</a>{/if}{$myfiles_delim}
		    <a href="{$admin_url}/textads/edit/{$adsview[aa].aid}">{$adm_vadsaction3}</a>{$myfiles_delim}
    		    <a href="{$admin_url}/textads/delete/{$adsview[aa].aid}" onClick='Javascript:return confirm("{$adm_vadsdelmsg}");'>{$adm_vadsaction4}</a>
    		    </td>
		</tr>
		{/section}
		<tr>
