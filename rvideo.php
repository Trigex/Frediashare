...
if ($_REQUEST[rel]!="")
{
    ...
    $sql="SELECT * from files_video where vtype='public' and vkey!='$key' and active='1' and is_inapp='no' and (vtags like '%$relv[0]%' $chnl) order by views desc limit 0,50";
    $smarty->assign('let', 'l');
...
elseif ($_REQUEST[user]!="")
{
    ...
    $list=key_to_info($key);
    $uid=$list[1];
    $sql="select * from files_video where vtype='public' and vkey!='$key' and active='1' and is_inapp='no' and uid='$uid' order by views desc limit 0,50";
    $smarty->assign('let', 'u');
    $smarty->assign('vidid', $list[0]);
}
