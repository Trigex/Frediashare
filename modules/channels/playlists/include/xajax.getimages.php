...
if(empty($_SESSION['rpagnr'])) { $_SESSION['ppagnr'] = 1; }
...
function choose_thumb($type, $vidid, $thnr, $vkey) {
    ...
    $fu = $conn->execute("select uid,vflvname from files_".$type." where vid='".$vidid."'");
    $fuid = $fu->fields['uid'];
    $flvname = $fu->fields['vflvname'];
    $tt = "<img alt=\"\" title=\"\" class=\"thumb\" width=\"".$config['img_max_width']."\" height=\"".$config['img_max_height']."\" src=\"".$config['base_url']."/media/files_thumbnail/user".$fuid."/".$flvname."\">";
    
    $oRes = new xajaxResponse();
    if ( $vidid != '' ) $fr = $conn->execute("update playlist_".$type." set thumb='".$vidid."' where vkey='".$vkey."';");
    if ( $conn->Affected_Rows() > 0 ) { 
	//$msgdiv = '<div id="newhide" style="padding: 5px; text-align: center; border: 1px solid green; color: green;">'.$lang['plist_txt52'].'<a href="javascript:void(0)" onclick="HideContent(&#39;getiplthumb&#39;); HideContent(&#39;newhide&#39;);">&nbsp;(X)</a></div>';
	$msgdiv = show_msg ( $lang['plist_txt52'] );
	$oRes->addAssign("preloader","style.display","block"); 
	$oRes->addAssign("iplthumb","innerHTML",$tt); 
...
function listare_resp($afisare_pagina = 1, $vidid) {
    ...
    $vu = $conn->execute("select uid from playlist_files where vkey='$vidid' and ptype='image';");
    $vuid = $vu->fields['uid'];
    
    mysql_connect($db_host, $db_user, $db_pass);
    @mysql_select_db($db_name) or die( "No db selected!");
    
    $bvsql = "select s.*,v.* from files_image v, playlist_files s where v.active='1' and v.is_inapp='no' and v.vtype='public' and v.vid=s.vid and s.ptype='image' and s.active='1' and s.vkey='$vidid' order by s.position asc;";
    ...
    {
	$poze[$i] = "media/files_thumbnail/user".$uid[$i]."/".$flv[$i];
    }
    $oRes = new xajaxResponse();
...
// LIST CYCLE
	...
	$rnd = $vid[$k].substr(md5($vid[$k]),12,7);
	...
	$html.="<td valign=\"top\" class=\"p2\" align=\"left\">
    ...
    $html.= '</table><div><input type="button" class="fb" onclick="sh(&#39;preloader&#39;); xajax_choose_thumb(&#39;image&#39;, document.getElementById(&#39;thisthumb&#39;).value, document.getElementById(&#39;thisthumbnr&#39;).value, &#39;'.$vidid.'&#39;);" value="'.$lang['plist_txt51'].'" style="width: 70px;">'.$lang['plist_txt10'].'<a href="javascript:void(0)" onclick="HideContent(&#39;getiplthumb&#39;);">'.$lang['comm_cancel'].'</a><input type="hidden" id="thisthumbnr" value=""><input type="hidden" id="thisthumb" value=""></div>';
