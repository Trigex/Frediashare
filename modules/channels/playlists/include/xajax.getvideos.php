...
function choose_thumb($type, $vidid, $thnr, $vkey) {
    ...
    $fuid = $fu->fields['uid'];
    $tt = "<img alt=\"\" title=\"\" class=\"thumb\" width=\"".$config['img_max_width']."\" height=\"".$config['img_max_height']."\" src=\"".$config['base_url']."/media/files_thumbnail/user".$fuid."/".$thnr."_".$vidid.substr(md5($vidid),13,7).".jpg\">";
    
    $oRes = new xajaxResponse();
    if ( $vidid != '' ) $fr = $conn->execute("update playlist_".$type." set thumb='".$vidid."' where vkey='".$vkey."';");
    if ( $conn->Affected_Rows() > 0 ) { 
	//$msgdiv = '<div id="newhide" style="padding: 5px; text-align: center; border: 1px solid green; color: green;">'.$lang['plist_txt52'].'<a href="javascript:void(0)" onclick="HideContent(&#39;getvplthumb&#39;); HideContent(&#39;newhide&#39;);">&nbsp;(X)</a></div>';
	$msgdiv = show_msg ( $lang['plist_txt52'] );
	$oRes->addAssign("preloader","style.display","block"); 
	$oRes->addAssign("vplthumb","innerHTML",$tt); 
...
function listare_resp($afisare_pagina = 1, $vidid) {
    ...
    $vu = $conn->execute("select uid from playlist_files where vkey='$vidid' and ptype='video';");
    $vuid = $vu->fields['uid'];
    
    mysql_connect($db_host, $db_user, $db_pass);
    @mysql_select_db($db_name) or die( "No db selected!");
    
    $bvsql = "select s.*,v.* from files_video v, playlist_files s where v.active='1' and v.is_inapp='no' and v.vtype='public' and v.vid=s.vid and s.ptype='video' and s.active='1' and s.vkey='$vidid' order by s.position asc;";
...
// LIST CYCLE
    ...
    $html.= '</table><div><input type="button" class="fb" onclick="sh(&#39;preloader&#39;); xajax_choose_thumb(&#39;video&#39;, document.getElementById(&#39;thisthumb&#39;).value, document.getElementById(&#39;thisthumbnr&#39;).value, &#39;'.$vidid.'&#39;);" value="'.$lang['plist_txt51'].'" style="width: 70px;">'.$lang['plist_txt10'].'<a href="javascript:void(0)" onclick="HideContent(&#39;getvplthumb&#39;);">'.$lang['comm_cancel'].'</a><input type="hidden" id="thisthumbnr" value=""><input type="hidden" id="thisthumb" value=""></div>';
