...
function listare_respdel($type, $vidid, $tovid) {
    ...
    $fr = $conn->execute("select s.*,v.* from files_image v, file_responses s where v.active='1' and v.is_inapp='no' and v.vtype='public' and s.rtovid='$tovid' and v.vid=s.rtovid and s.active='1' and s.approved='1' and s.rtype='image' and (v.is_fileresp='yes' or v.is_fileresp='app') order by s.rtime desc;");
    $tt = '('.$fr->recordcount().')';
...
function listare_resp($afisare_pagina = 1, $vidid) {
    ...
    $vu = $conn->execute("select uid from files_image where vid='$vidid';");
    $vuid = $vu->fields['uid'];
    
    mysql_connect($db_host, $db_user, $db_pass);
    @mysql_select_db($db_name) or die( "No db selected!");
    
    $bvsql = "select s.*,v.* from files_image v, file_responses s where v.active='1' and v.is_inapp='no' and v.vtype='public' and v.vid=s.rvid and s.rtovid='$vidid' and s.active='1' and s.approved='1' and s.rtype='image' order by s.rtime desc;";
...
// LIST CYCLE
	...
	if ( $_SESSION['UID'] != '' and $_SESSION['UID'] == $vuid ) {
            $b = "<div><input style=\"font-size: 9px; padding: 1px;\" type=\"button\" class=\"fb\" name=\"delresp\" value=\"".$lang['fresp_txt26']."\" onclick=\"if ( confirm ( '".$lang['fresp_txt27']."' ) ) { sh('preloader'); xajax_listare_respdel('image', '".$vid[$k]."', '".$vidid."'); }\"></div>";
        $html.="
                        <tr>
