...
if ($_REQUEST[vcancel]) { header("Location: $config[base_url]/my_videos"); exit; } 
	...
	if ($err=="")
	{
	    ...
	    if ( $err == '' ) {
            ...
            if ($_REQUEST[actions]==$lang['act_addfav']) {//add to favorites
                foreach($farr as $value) {
                    //$fu = $conn->execute("select uid from files_video where vid='".filter_title($value)."'"); $fuid = $fu->fields['uid'];
                    $conn->execute("insert into fav_video set vid='".filter_title($value)."', uid='$_SESSION[UID]', from_uid='$_SESSION[UID]';");
            ...
            if ($_REQUEST[actions]==$lang['qlist_txt1']) {//add to quicklist
                foreach($farr as $value) {
                    ...
                    $conn->execute("insert into quicklist_video set uid='".$_SESSION['UID']."', vid='".filter_title($value)."', addtime='".time()."', from_uid='".$_SESSION['UID']."';");
                }
            } 
	    if ($_REQUEST[actions]==$lang['act_markpub']) //mark public
	    {
		foreach($farr as $value) { $conn->execute("update files_video set vtype='public' where vtype='private' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_markpriv']) //mark private
	    {
		foreach($farr as $value) { $conn->execute("update files_video set vtype='private' where vtype='public' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark1comm']) //enable comments
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_comm='yes' where is_comm='no' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark0comm']) //disable comments
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_comm='no' where is_comm='yes' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['macts1']) //enable comment rating
           {
               foreach($farr as $value) { $conn->execute("update files_video set is_commrate='yes' where is_commrate!='' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
           }
           if ($_REQUEST[actions]==$lang['macts2']) //disable comment rating
           {
               foreach($farr as $value) { $conn->execute("update files_video set is_commrate='no' where is_commrate!='' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
           }
           if ($_REQUEST[actions]==$lang['macts3']) //enable file responses
           {
               foreach($farr as $value) { $conn->execute("update files_video set is_fileresp='yes' where is_fileresp!='' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
           }
           if ($_REQUEST[actions]==$lang['macts4']) //disable file responses
           {
               foreach($farr as $value) { $conn->execute("update files_video set is_fileresp='no' where is_fileresp!='' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
           }
	    if ($_REQUEST[actions]==$lang['act_mark1rate']) //enable ratings
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_rated='yes' where is_rated='no' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark0rate']) //disable ratings
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_rated='no' where is_rated='yes' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark1emb']) //enable embedding
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_embed='yes' where is_embed='no' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark0emb']) //disable embedding
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_embed='no' where is_embed='yes' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark1bm']) //enable bookmarking
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_bm='yes' where is_bm='no' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['act_mark0bm']) //disable bookmarking
	    {
		foreach($farr as $value) { $conn->execute("update files_video set is_bm='no' where is_bm='yes' and vid='".filter_title($value)."' and uid='$_SESSION[UID]'"); }
	    }
	    if ($_REQUEST[actions]==$lang['inbox_itblact6']) //delete selected
	    {
		foreach($farr as $value)
		{
		    delete_vdo(filter_title($value),$_SESSION[UID]); 
		    /*
		    if ($config[delete_files]=="0")
		    {
			$conn->execute("delete from history_video where vid='".filter_title($value)."'");
			$conn->execute("delete from fav_video where vid='".filter_title($value)."'");
			$conn->execute("delete from request_video_F where vid='".filter_title($value)."'");
			$conn->execute("delete from request_video_I where vid='".filter_title($value)."'");
			$conn->execute("delete from comm_vid where vid='".filter_title($value)."'");
			$conn->execute("delete from comm_rates where vid='".filter_title($value)."' and type='video'");
			$conn->execute("delete from files_video where vid='".filter_title($value)."' and uid='$_SESSION[UID]'");
			$conn->execute("update pmessages set fvideo='0' where fvideo='$value'");
			$conn->execute("update members set files_video=files_video-1 where uid='$_SESSION[UID]'");
		    }
		    if ($config[delete_files]=="1") 
		    { 
			delete_vdo($value,$_SESSION[UID]); 
			$conn->execute("update pmessages set fvideo='0' where fvideo='$value'");
			$conn->execute("update members set files_video=files_video-1 where uid='$_SESSION[UID]'");
		    }
		    */
    ...
    // paging begins
    $listing_per_page = $config[paging_myvideos];
    if($_REQUEST[page]=="") $page = 1;
    else $page = $_REQUEST[page];
    $sql = "SELECT count(*) as total from files_video where uid='$_SESSION[UID]' $tq $qu";
    ...
    $sql="select * from files_video WHERE uid='$_SESSION[UID]' $tq $qu limit $startfrom, $listing_per_page";
    $rs=$conn->execute($sql);
	...
	{
	    if ($type!="") $page_no .= " <a href='$config[base_url]/my_videos/$type$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    else $page_no .= " <a href='$config[base_url]/my_videos/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	}
    }
    if($page_no!="") $link = "$lang[gen_page] $page_no $lang[gen_pageof]";
    else $link = $lang['myvideos_none'];
	...
	if ($type!="") $prevlink="<a href='$config[base_url]/my_videos/$type$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>";
	else $prevlink="<a href='$config[base_url]/my_videos/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>";
	
	if ($type!="") $nextlink="<a href='$config[base_url]/my_videos/$type$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
	else $nextlink="<a href='$config[base_url]/my_videos/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
    ...
    $smarty->assign('vids',$vdo);
    $smarty->assign('vd',$vdo);
    $smarty->assign('start_num',$start_num);
    $smarty->assign('end_num',$end_num);
    $smarty->assign('total',$total);
    
    //my tags
    $mt="select vtags from files_video where uid='$_SESSION[UID]' $tq $qu limit $startfrom, $listing_per_page";
    $tags=tags($mt);
    $smarty->assign('mytags',$tags);

    if($_REQUEST[type]=="dy")
    {
	set_btn("bhome"); set_sbtn("myvid"); set_title($lang['title_myfilesconfirm']);
	$msg = $lang['not_myvideo1'];
	$smarty->assign('msg',$msg);
	set_title($lang['title_myfilesconfirm']);
	$smarty->display('main_header.tpl'); $smarty->display('main_myvideo.tpl'); $smarty->display('footer.tpl'); exit;
    }
    if($_REQUEST[type]=="dn")
    {
	set_btn("bhome"); set_sbtn("myvid"); set_title($lang['title_myfileserror']);
	$err = $lang['err_myvideo1'];
	...
	$list=key_to_info($vid);
	$vidid=$list[0];
	
	$conn->execute("delete from history_video where vid='$vidid'");
	$conn->execute("delete from fav_video where vid='$vidid'");
	$conn->execute("delete from request_video_F where vid='$vidid'");
	$conn->execute("delete from request_video_I where vid='$vidid'");
	$conn->execute("delete from comm_vid where vid='$vidid'");
	
	if ($config[delete_files]=="0")
	{
//	    $conn->execute("update members set files_video='files_video-1' where uid='$_SESSION[UID]'");
	    $sql="delete from files_video where vkey='$vid' and uid='$_SESSION[UID]'";
	    $res=$conn->execute($sql);
	    if (mysql_affected_rows()>0) 
	    {
		header("Location: $config[base_url]/my_videos/confirmation");
		exit;
	    }
	    else
	    {
	...
	if ($config[delete_files]=="1")
	{
	    $msg = delete_vdo($vidid,$_SESSION[UID]);
	    if ($msg=="yes") 
	    { 
		header("Location: $config[base_url]/my_videos/confirmation");
		exit; 
	    }
	    if ($msg=="no") { header("Location: $config[base_url]/my_videos/error"); exit; }
	}
    }
    ...
    if ($config[same_title_uploads]=="0")                                                                                                                                                                      
    {                                                                                                                                                                                                          
	$vt=ereg_replace("_{1,}", " ", $_REQUEST[vid]);
	$tq="select vkey from files_video where vtitle='$vt'";
	$rq=$conn->execute($tq);
	$vid=$rq->fields[vkey];
    } else { $vid=$_REQUEST[vid]; }
	...
	if ($err=="")
	{
	    ...
	    if($_FILES['upfile_1']['tmp_name']!="" && $err=="")
	    {
		$mv=$conn->execute("select vid from files_video where vkey='$vid' and uid='$_SESSION[UID]'");
		$myvid=$mv->fields[vid];
		$rnd=$myvid.substr(md5($myvid),13,7);
		...
		if(rename($src, $dst))
		{
		    ...
		    $output1 = $dst1;
		    if ($phpThumb->GenerateThumbnail()) { $phpThumb->RenderToFile($output1); @chmod($output1, 0666); $msg = $lang['not_myvideo2']; $smarty->assign('msg',$msg); }
		...
		if(rename($src, $dst))
		{
		    ...
		    $output1 = $dst1;
		    if ($phpThumb2->GenerateThumbnail()) { $phpThumb2->RenderToFile($output1); @chmod($output1, 0666); $msg = $lang['not_myvideo2']; $smarty->assign('msg',$msg); }
		...
		if(rename($src, $dst))
		{
		    ...
		    $output1 = $dst1;
		    if ($phpThumb3->GenerateThumbnail()) { $phpThumb3->RenderToFile($output1); @chmod($output1, 0666); $msg = $lang['not_myvideo2']; $smarty->assign('msg',$msg); }
	...
	$sql="select * from files_video where vkey='$vid' and uid='$_SESSION[UID]'";
	$rs=$conn->execute($sql);
	...
	set_title($lang['title_myfileseditv']);
	$smarty->assign('err',$err);
	$smarty->assign('msg',$msg);
	$smarty->assign('vid',$vid);
	$smarty->display('main_header.tpl');
	$smarty->display('myvideo_edit.tpl');
	...
	if(count($_REQUEST[categlist])<1) $err = $lang['err_myvideo2'];
	elseif ( ( $config['multi_categ_max'] != '0' and $config['multi_categ_max'] != '' ) and count ( $vcategs ) > $config['multi_categ_max'] ) $err = $lang['up_errup1'];
	
	if ($vtags=="") $err = $lang['err_myvideo3'];
	elseif (strlen($vtags)<$config[fi_tamin] || strlen($vtags)>$config[fi_tamax]) $err = $lang['err_myvideo4'];

        if ($vdescr=="") $err = $lang['err_myvideo6'];
        elseif (strlen($vdescr)<$config[fi_demin] || strlen($vdescr)>$config[fi_demax]) $err = $lang['err_myvideo7'];

        if ($vtitle=="") $err = $lang['err_myvideo9'];
        elseif (strlen($vtitle)<$config[fi_timin] || strlen($vtitle)>$config[fi_timax]) $err = $lang['err_myvideo10'];
	
	if ( $config['same_title_uploads'] == '0' ) {
	    $sql="select * from files_video where vtitle='$vtitle' and vkey!='$vid'";
	    $rs=$conn->execute($sql);
    	    if($rs->recordcount()>0) $err=$lang['err_upvideo2'];
	}
	...
	if ($err=="")
	{
	    ...
	    $sql = "update files_video set $fc vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', vtype='$vtype' where vkey='$vid' and uid='$_SESSION[UID]'";
	    $mrs=$conn->execute($sql);
	    $msg = $lang['not_myvideo2'];
	    $smarty->assign('msg',$msg);
	    ...
	    $sql = "SELECT count(*) as total from files_video where uid='$_SESSION[UID]'";
	    $ars = $conn->Execute($sql);
	    $total = $ars->fields['total'];
	    $grandtotal = $total;
	    $tpage = ceil($total/$listing_per_page);
	    ...
	    $sql="select * from files_video WHERE uid='$_SESSION[UID]' order by vtitle limit $startfrom, $listing_per_page";
	    $rs=$conn->execute($sql);
	    if($rs->recordcount()>0)$vdo = $rs->getrows();
	    $start_num=$startfrom+1;
	    $end_num=$startfrom+$rs->recordcount();
		...
		$nextpage=$page+1;
		$prevpage=$page-1;
		$prevlink="<a href='$config[base_url]/my_videos/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>";
		$nextlink="<a href='$config[base_url]/my_videos/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
		if($page==$tpage) $link="$prevlink&nbsp;&nbsp;&nbsp;".$link;
		elseif($tpage>$page && $page>1) $link="$prevlink&nbsp;&nbsp;&nbsp;".$link."&nbsp;&nbsp;&nbsp;$nextlink";
		elseif($tpage>$page && $page<=1) $link.="&nbsp;&nbsp;&nbsp;$nextlink";
	    }
	    ...
	    $smarty->display('main_header.tpl');
	    $smarty->display('main_myvideo.tpl');
	    $smarty->display('footer.tpl');
	    exit;
	}
...
$smarty->assign('qtotal', $qtotal); $smarty->assign('atotal', $atotal); $smarty->assign('itotal', $itotal); $smarty->assign('vtotal', $vtotal);
    set_btn("bhome");
    set_sbtn("myvid");
    set_title($lang['title_myfilesvideo']);
    $smarty->assign('err',$err);
    $smarty->assign('msg',$msg);
    $smarty->display('main_header.tpl');
    $smarty->display('main_myvideo.tpl');
