...
if ($config['enable_videos']=="0") { header("Location: $config[base_url]/login"); exit; }
if (($_SESSION[UID]=="") && $config[guests_video_view]=="0") { header("Location: $config[base_url]/login?next=videos"); exit; }
if (($_SESSION[UID]=="") && ($_REQUEST[type]!="" && $config[guests_file_sorting]=="0") && $_REQUEST['category'] == '') { header("Location: $config[base_url]/login?next=videos/$_REQUEST[type]"); exit; }
if (($_SESSION[UID]=="") && ($_REQUEST[type]!="" && $config[guests_file_sorting]=="0") && $_REQUEST['category'] != '') { header("Location: $config[base_url]/login?next=categories/video/$_REQUEST[category]/recent"); exit; }
...
if ($_REQUEST[user]!="")
{
    if ($config[guests_profile_view] !="1") { check_login('user_videos/'.$_REQUEST['user']); }
    if ( $config['enable_channels'] == '1' ) { header("Location: $config[base_url]/user/$_REQUEST[user]&view=videos"); exit; }
    $user=$_REQUEST[user];
...
{
    if ($config[guests_categories_view]!="1" && $_SESSION[UID]=="") { header("Location: $config[base_url]/login?next=categories/video/$_REQUEST[category]/recent"); exit; }
...
// paging
$listing_per_page = $config[paging_videos];
if($_REQUEST[page]=="")
$page = 1;
else
$page = $_REQUEST[page];
...
if ($frs->recordcount()>0) { $sql = "SELECT count(*) as total from files_video where active='1' $tq $qu $que limit $listing_per_page"; }
elseif (($uid==$_SESSION[UID]) && $uid!="") { $sql = "SELECT count(*) as total from files_video where active='1' $tq $qu $que limit $listing_per_page"; }
else { 
$sql = "SELECT count(*) as total from files_video where vtype='public' $active $tq $qu $que limit $listing_per_page"; 
}
...
for($i=1; $i<=$tpage; $i++)
{
    ...
    {
	if ($_REQUEST[user]!="") { $page_no .= " <a href='$config[base_url]/user_videos/$user/$cat$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> "; }
	elseif ($_REQUEST[category]!="") { $page_no .= " <a href='$config[base_url]/categories/video/$categ/$cat$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> "; }
	else { $page_no .= " <a href='$config[base_url]/videos/$cat$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> "; }
    }
}

if($page_no!="") $link = "$lang[gen_page] $page_no $lang[gen_pageof]";
else $link = $lang['videos_none'];
...
if($tpage>1)
{
    $nextpage=$page+1;
    $prevpage=$page-1;
    if ($_REQUEST[user]!="") { $prevlink="<a href='$config[base_url]/user_videos/$user/$cat$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; }    
    elseif ($_REQUEST[category]!="") { $prevlink="<a href='$config[base_url]/categories/video/$categ/$cat$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; }    
    else { $prevlink="<a href='$config[base_url]/videos/$cat$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; }
    
    if ($_REQUEST[user]!="") { $nextlink="<a href='$config[base_url]/user_videos/$user/$cat$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; }    
    elseif ($_REQUEST[category]!="") { $nextlink="<a href='$config[base_url]/categories/video/$categ/$cat/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; }    
    else { $nextlink="<a href='$config[base_url]/videos/$cat$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; }
...
//get categories
$csql="select * from categories where active='1' and video_uploads='yes' order by name asc";
...
//friend check + video tags
if ($frs->recordcount()>0) { $sql="SELECT * from files_video where active='1' $tq $qu $que limit $startfrom, $listing_per_page"; }
else 
{ 
    if ($_SESSION[UID]=="") { $sql="SELECT * from files_video where vtype='public' $active $tq $qu $que limit $startfrom, $listing_per_page"; }
    else 
    { 
	if ($own=="1") { $active="active='1'"; $sql="SELECT * from files_video where $active $tq $qu $que limit $startfrom, $listing_per_page"; }
	else { $sql="SELECT * from files_video where vtype='public' $active $tq $qu $que limit $startfrom, $listing_per_page"; }
    }
}
$vsql="select vtags from files_video where vtype='public' and active='1' and is_inapp='no' $tq $qu $que limit $startfrom, $listing_per_page";
$vtags = tags($vsql);
...
set_btn("bvideo"); set_sbtn("bvideo"); set_title($cat1.$lang['title_viewvideos']);
$smarty->display('main_header.tpl');
$smarty->display('main_videos.tpl');
