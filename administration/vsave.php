...
if (isset($_REQUEST['duration'])) $dur=mysql_real_escape_string($_REQUEST['duration']);
if (isset($_REQUEST['rec'])) $vrec=filter_title($_REQUEST['rec']);
...
if ($vsubmit!="")
{
    ...
    elseif (!is_numeric($dur) or $dur < 0) { echo show_err ( $lang['admerr_setoptions5'] ); exit; }
    elseif (strtotime($date) > strtotime(date("Y-m-d"))) { echo show_err ( $lang['admerr_setoptions5'] ); exit; }
    
    if($vrec=="yes") 
    { 
    	$q1="update files_video set is_recommended='no' where vkey!='$vkey'";
	$r1=mysql_query($q1);
	$sql="update files_video set adddate='$date', addtime='".strtotime($date." ".date("H:i:s"))."', vduration='$dur', vfav='$favs', views='$views', vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', active='$vappr', is_featured='$vfeat', vtype='$vtype', is_recommended='$vrec',is_inapp='$vinapp' where vkey='$vkey'"; 
    }
    elseif ($vfeat=="yes")
    {
	$q2="update request_video_F set active='1' where vid='$vid'";
	$r2=mysql_query($q2);
	{ $sql="update files_video set adddate='$date', addtime='".strtotime($date." ".date("H:i:s"))."', vduration='$dur', vfav='$favs', views='$views', vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', active='$vappr', is_featured='$vfeat', vtype='$vtype', is_recommended='$vrec',is_inapp='$vinapp' where vkey='$vkey'"; }
    }
    else { $sql="update files_video set adddate='$date', addtime='".strtotime($date." ".date("H:i:s"))."', vduration='$dur', vfav='$favs', views='$views', vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', active='$vappr', is_featured='$vfeat', vtype='$vtype', is_recommended='$vrec',is_inapp='$vinapp' where vkey='$vkey'"; }
    $res=mysql_query($sql);
    
    if ($solved=="1")
    {
	$sq="update request_video_F set solved='1' where vid='$vid'";
	$res=mysql_query($sq);
    }
    if ($solvei=="1")
    {
	$sq="update request_video_I set solved='1' where vid='$vid'";
	$res=mysql_query($sq);
    }
    if(mysql_affected_rows()>0) { echo show_msg ( $lang['admnot_setoptions1'] ); }
    
} else { echo show_msg ( $lang['admnot_setoptions2'] ); exit; }
?>