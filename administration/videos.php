...
$tbl="files_video"; $qu="order by vtitle";
...
if ($_REQUEST['goact']!="" && $err=="")
{
	...
	if ($_REQUEST['actions'] == $lang['admact_mcomm']) //delete comments
	{
		...
		if ($tr > 0)
		{
		    $conn->execute("update files_video set comments=comments-$tr where vid='".filter_title($value)."'"); 
	...
	if ($_REQUEST['actions'] == $lang['inbox_itblact6']) //delete selected
	{
	    foreach($farr as $value) 
	    { 
		$r = $conn->execute("select uid from files_video where vid='$value'");
		delete_vdo($value, $r->fields['uid']);
//		$conn->execute("update pmessages set fvideo='0' where fvideo='$value'");
//		$conn->execute("update members set files_video=files_video-1 where uid='".$r->fields['uid']."'");
	    }
	}
...
if($type!="")
{
    ...
    elseif ($type=="recommended") { if($user!="") $qu="where uid='$userid' and is_recommended='yes'"; elseif($categ!="") $qu="where vcategs like '%$cid%' and is_recommended='yes'"; else $qu="where is_recommended='yes' order by vtitle"; $pg="recommended"; }
...
if ($del!="")
{
    if(!key_to_info($del)) $err=$lang['err_videos2'];
    if($err=="")
    {
	$info=key_to_info($del);
	$vidid=$info[0];
	$vuid=$info[1];
	
	$msg=delete_vdo($vidid,$vuid);
	if ($msg=="yes") { $conn->execute("update members set files_video=files_video-1 where uid='$vuid'"); $msg=$lang['admnot_delete']; }
...
if ($id!="")
{
    if (!key_to_info($id)) { $err=$lang['err_videos2']; }
    if ($err=="")
    {
	$tbl="files_video";
	$list=key_to_info($id);
...
for($i=1; $i<=$tpage; $i++)
{
	...
	if ($type!="") 
	{ 
	    if($user!="") $page_no .= " <a href='$config[admin_url]/videou/$user/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    elseif($categ!="") $page_no .= " <a href='$config[admin_url]/videoc/$categ/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    else $page_no .= " <a href='$config[admin_url]/videos/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	}
	else 
	{ 
	    if($user!="") $page_no .= " <a href='$config[admin_url]/videou/$user$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    elseif($categ!="") $page_no .= " <a href='$config[admin_url]/videoc/$categ$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    else $page_no .= " <a href='$config[admin_url]/videos$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	}
    }
}
if($page_no!="") $link = "$lang[gen_page] $page_no $lang[gen_pageof]";
else $link = $lang['adm_novideos'];
...
if($tpage>1)
{
    ...
    if ($type!="") 
    { 
	if($user!="") $prevlink="<a href='$config[admin_url]/videou/$user/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	elseif($categ!="") $prevlink="<a href='$config[admin_url]/videoc/$categ/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	else $prevlink="<a href='$config[admin_url]/videos/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
    }
    else 
    { 
	if($user!="") $prevlink="<a href='$config[admin_url]/videou/$user$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	elseif($categ!="") $prevlink="<a href='$config[admin_url]/videoc/$categ$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	else $prevlink="<a href='$config[admin_url]/videos$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 
    }


    if ($type!="") 
    { 
	if($user!="") $nextlink="<a href='$config[admin_url]/videou/$user/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	elseif($categ!="") $nextlink="<a href='$config[admin_url]/videoc/$categ/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 		
	else $nextlink="<a href='$config[admin_url]/videos/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
    }
    else 
    { 
	if($user!="") $nextlink="<a href='$config[admin_url]/videou/$user$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	elseif($categ!="") $nextlink="<a href='$config[admin_url]/videoc/$categ$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	else $nextlink="<a href='$config[admin_url]/videos$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
    }
...
$smarty->assign('vids',$vid);

set_btn("adm_video"); set_sbtn("allv"); $smarty->assign('is_admin', '1');
$smarty->assign('err',$err);
$smarty->assign('msg',$msg);
$smarty->display('administration/main_header.tpl');
$smarty->display('administration/main_videos.tpl');
