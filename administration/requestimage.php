...
//innapropriate request from view image page
$innap_btn=filter_title($_POST['innap_btn']);
//feature request from view image page
$feat_btn=filter_title($_POST['feat_btn']);
//favorites request from view image page
...
if ($fav_btn!='')
{
    $list=key_to_info_image($key2);
//    if($_SESSION[UID]==$list[1]) { echo show_err ( $lang[err_addfav1] ); exit; } 
    
    $sql="insert into fav_image set uid=$_SESSION[UID],vid=$list[0],from_uid='$list[1]'";
    $conn->execute($sql);
    if(mysql_affected_rows()>=1) { 
	$sql1="update files_image set vfav=vfav+1 where vid='$list[0]'";
	$conn->execute($sql1);
	if(mysql_affected_rows()>=1) { echo show_msg ( $lang[not_addfav1] ); exit; }
...
{ 
	...
	$trs=$conn->execute("select * from request_image_I where vid='$vidid' and solved='0'");
        if ($trs->recordcount()>0) { echo show_err ( $lang[err_inapp5] ); exit; }

        $sql="insert into request_image_I set vid='$vidid', uid='$_SESSION[UID]', from_uid='$vuid', date='".date("Y-m-d H:i:s")."' ,reason='$reason'";
	$rs=mysql_query($sql);
	$mid=mysql_insert_id(); 
...
if ($feat_btn != '' and $c == '')
{
    ...
    {
	$reason=$feat;
        $list=key_to_info_image($key1);
        $vidid=$list[0];
        $vuid=$list[1];
	$feat=$list[13]; if($feat=="yes") { echo show_err ( $lang[err_inapp7] ); exit; }
	
	$trs=$conn->execute("select * from request_image_F where vid='$vidid'");
	if ($trs->recordcount()>0) { echo show_err ( $lang[err_inapp5] ); exit; }

        $sql="insert into request_image_F set vid='$vidid', uid='$_SESSION[UID]', from_uid='$vuid', date='".date("Y-m-d H:i:s")."' ,reason='$reason'";
	$rs=mysql_query($sql);
	$mid=mysql_insert_id(); 
