...
if ($vsubmit!="")
{
    ...
    if ($vfeat=="yes")
    {
	$q2="update request_audio_F set active='1' where vid='$vid'";
	$r2=mysql_query($q2);
	{ $sql="update files_audio set adddate='$date', addtime='".strtotime($date." ".date("H:i:s"))."', vduration='$dur', vfav='$favs', views='$views', vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', active='$vappr', is_featured='$vfeat', vtype='$vtype', is_recommended='$vrec',is_inapp='$vinapp' where vkey='$vkey'"; }
    }
    else { $sql="update files_audio set adddate='$date', addtime='".strtotime($date." ".date("H:i:s"))."', vduration='$dur', vfav='$favs', views='$views', vtitle='$vtitle', vdescr='$vdescr', vtags='$vtags', vcategs='0,$listch,0', active='$vappr', is_featured='$vfeat', vtype='$vtype', is_recommended='$vrec',is_inapp='$vinapp' where vkey='$vkey'"; }
    $res=mysql_query($sql);
    
    if ($solved=="1")
    {
	$sq="update request_audio_F set solved='1' where vid='$vid'";
	$res=mysql_query($sq);
    }
    if ($solvei=="1")
    {
	$sq="update request_audio_I set solved='1' where vid='$vid'";
	$res=mysql_query($sq);
    }
