...
{
    global $conn, $config;
    $del = $conn->execute("select image from watermark_audio WHERE wid='$id'");
    $del_pic = $del->fields['image'];
    $del_file = $config[dir_fp]."/wms_audio/".$del_pic;
    if (file_exists($del_file)) 
    { 
	if(@unlink($del_file))
	{
	    $conn->execute("delete from watermark_audio where wid='$id'");
	}
    }
    else $err='no';
...
if ($_REQUEST[goact]!="" && $err=="")
{
    ...
    if ($err=="")
    {
        ...
        {
            foreach($_REQUEST['mid'] as $value) { $conn->execute("update watermark_audio set active='1' where wid='$value' and active='0'"); }
        }
        if ($_REQUEST[actions]==$lang['act_frdisable']) //disable selected
        {
            foreach($_REQUEST['mid'] as $value) { $conn->execute("update watermark_audio set active='0' where wid='$value' and active='1'"); }
        }
        if ($_REQUEST[actions]==$lang['inbox_itblact6']) //delete selected
        {
            foreach($_REQUEST['mid'] as $value) { del_wma($value); }
        }
...
{
    $ret=del_wma($_REQUEST['id']);
    if ($ret=='yes') { header("Location: $config[admin_url]/watermark_audio/saved"); exit; }
    else illegal_opa();
}
...
$page = $_REQUEST[page];
$sql = "SELECT count(*) as total from watermark_audio $query";
...
for($i=1; $i<=$tpage; $i++)
{
    ...
    else $page_no .= " <a href='$config[admin_url]/watermark_audio/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
}
...
if($tpage>1)
{
    $nextpage=$page+1;
    $prevpage=$page-1;
    $prevlink="<a href='$config[admin_url]/watermark_audio/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>";
    $nextlink="<a href='$config[admin_url]/watermark_audio/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
...
$sql="SELECT * from watermark_audio $query limit $startfrom, $listing_per_page";
$rs = $conn->Execute($sql);
...
$smarty->display("administration/watermark_main_audio.tpl");
