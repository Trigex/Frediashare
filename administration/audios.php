...
$tbl="files_audio"; $qu="order by vtitle";
...
if ($_REQUEST['goact']!="" && $err=="")
{
	...
	if ($_REQUEST['actions'] == $lang['admact_mcomm']) //delete comments
	{
		...
		if ($tr > 0)
		{
		    $conn->execute("update files_audio set comments=comments-$tr where vid='".filter_title($value)."'"); 
	...
	if ($_REQUEST['actions'] == $lang['inbox_itblact6']) //delete selected
	{
	    foreach($farr as $value) 
	    { 
		$r = $conn->execute("select uid from files_audio where vid='$value'");
		delete_ado($value, $r->fields['uid']);
//		$conn->execute("update pmessages set faudio='0' where faudio='$value'");
//		$conn->execute("update members set files_audio=files_audio-1 where uid='".$r->fields['uid']."'");
	    }
	}
...
if($type!="")
{
    ...
    elseif ($type=="active") { if($user!="") $qu="where active='1' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and active='1' $tq order by vtitle"; else $qu="where active='1' $tq order by vtitle"; $pg="active"; }
    ...
    elseif ($type=="featured") { if($user!="") $qu="where is_featured='yes' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and is_featured='yes' $tq order by vtitle"; else $qu="where is_featured='yes' $tq order by vtitle"; $pg="featured"; }
    ...
    elseif ($type=="inactive") { if($user!="") $qu="where active='0' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and active='0' $tq order by vtitle"; else $qu="where active='0' $tq order by vtitle"; $pg="inactive"; }
    elseif ($type=="private") { if($user!="") $qu="where vtype='private' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and vtype='private' $tq order by vtitle"; else $qu="where vtype='private' $tq order by vtitle"; $pg="private"; }
    elseif ($type=="public") { if($user!="") $qu="where vtype='public' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and vtype='public' $tq order by vtitle"; else $qu="where vtype='public' $tq order by vtitle"; $pg="public"; }
    elseif ($type=="ratings") { if($user!="") $qu="where uid='$userid' and rate!='0' $tq order by rate desc"; elseif($categ!="") $qu="where vcategs like '%$cid%' and rate!='0' $tq order by rate desc"; else $qu="where rate!='0' $tq order by rate desc"; $pg="ratings"; }
    elseif ($type=="responses") { if($user!="") $qu="where uid='$userid' and responses!='0' $tq order by rate desc"; elseif($categ!="") $qu="where vcategs like '%$cid%' and responses!='0' $tq order by rate desc"; else $qu="where responses!='0' $tq order by responses desc"; $pg="responses"; }
    elseif ($type=="auditions") { if($user!="") $qu="where uid='$userid' $tq order by views desc"; elseif($categ!="") $qu="where vcategs like '%$cid%' $tq order by views desc"; else { $tq1 = str_replace("and", "where", $tq); $qu="$tq1 order by views desc"; } $pg="auditions"; }
    elseif ($type=="inappropriate") { if($user!="") $qu="where is_inapp='yes' and uid='$userid' $tq order by vtitle"; elseif($categ!="") $qu="where vcategs like '%$cid%' and is_inapp='yes' $tq order by vtitle"; else $qu="where is_inapp='yes' $tq order by vtitle"; $pg="inappropriate"; }
    else { illegal_opa(); }
}

if ($del!="")
{
    if(!key_to_info_audio($del)) $err=$lang['err_audios2'];
    if($err=="")
    {
	$info=key_to_info_audio($del);
	$vidid=$info[0];
	$vuid=$info[1];
	
	$msg=delete_ado($vidid,$vuid);
	if ($msg=="yes") { $conn->execute("update members set files_audio=files_audio-1 where uid='$vuid'"); $msg=$lang['admnot_delete']; }
...
if ($id!="")
{
    if (!key_to_info_audio($id)) { $err=$lang['err_audios2']; }
    if ($err=="")
    {
	$tbl="files_audio";
	$list=key_to_info_audio($id);
	...
	$sql="select * from files_audio where vkey='$id'";
	$rs=$conn->execute($sql);
...
for($i=1; $i<=$tpage; $i++)
{
	...
	if ($type!="") 
	{ 
	    if($user!="") $page_no .= " <a href='$config[admin_url]/audiou/$user/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    elseif($categ!="") $page_no .= " <a href='$config[admin_url]/audioc/$categ/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    else $page_no .= " <a href='$config[admin_url]/audios/$pg$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	}
	else 
	{ 
	    if($user!="") $page_no .= " <a href='$config[admin_url]/audiou/$user$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	    elseif($categ!="") $page_no .= " <a href='$config[admin_url]/audioc/$categ$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a>"; 	    
	    else $page_no .= " <a href='$config[admin_url]/audios$tstr/page$i'><span class=\"pag\" id=\"pg$i\">".$i."</span></a> ";
	}
    }
}
if($page_no!="") $link = "$lang[gen_page] $page_no $lang[gen_pageof]";
else $link = $lang['adm_noaudios'];
...
if($tpage>1)
{
    ...
    if ($type!="") 
    { 
	if($user!="") $prevlink="<a href='$config[admin_url]/audiou/$user/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	elseif($categ!="") $prevlink="<a href='$config[admin_url]/audioc/$categ/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	else $prevlink="<a href='$config[admin_url]/audios/$pg$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
    }
    else 
    { 
	if($user!="") $prevlink="<a href='$config[admin_url]/audiou/$user$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	elseif($categ!="") $prevlink="<a href='$config[admin_url]/audioc/$categ$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 	
	else $prevlink="<a href='$config[admin_url]/audios$tstr/page$prevpage'><span class=\"pag_prev\">$lang[gen_pageprev]</span></a>"; 
    }


    if ($type!="") 
    { 
	if($user!="") $nextlink="<a href='$config[admin_url]/audiou/$user/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	elseif($categ!="") $nextlink="<a href='$config[admin_url]/audioc/$categ/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 		
	else $nextlink="<a href='$config[admin_url]/audios/$pg$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
    }
    else 
    { 
	if($user!="") $nextlink="<a href='$config[admin_url]/audiou/$user$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	elseif($categ!="") $nextlink="<a href='$config[admin_url]/audioc/$categ$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>"; 	
	else $nextlink="<a href='$config[admin_url]/audios$tstr/page$nextpage'><span class=\"pag_prev\">$lang[gen_pagenext]</span></a>";
    }
...
$smarty->assign('auds',$vid);

set_btn("adm_audio"); set_sbtn("alla"); $smarty->assign('is_admin', '1');
$smarty->assign('err',$err);
$smarty->assign('msg',$msg);
$smarty->display('administration/main_header.tpl');
$smarty->display('administration/main_audios.tpl');
