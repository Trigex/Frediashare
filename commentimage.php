...
if ($comid!="")
{
    ...
    {
	$sql="delete from comm_img where comid='$comid'";	
	$rs=mysql_query($sql);
	if (mysql_affected_rows()>0)
	{
	    $sql1="update files_image set comments=comments-1 where vkey='$key'";
	    $rs1=mysql_query($sql1);
	    if (mysql_affected_rows()>0)
	    {
		 $conn->execute("delete from comm_rates where type='image' and comid='$comid';");
...
if ($send != '' and $c == '')
{
    ...
    elseif ($body=="") { echo show_err ( $lang[err_comm2] ); exit; }
    ...
    $vidid=$list[0];
    $vuid=$list[1];
    $sql="select uid from comm_img where active='1' and vid='$vidid' order by addtime desc";
    ...
    $rq = $conn->execute("select is_comm from files_image where vid='$vidid';");
    ...
    $sql="insert into comm_img set $comapp vid='$vidid', uid='$_SESSION[UID]', subject='$subject', comment='$body', addtime='".time()."'";
    $rs=mysql_query($sql);
    $mid=mysql_insert_id();
	...
	if (mysql_affected_rows()>0)
	{
	    $rm = $conn->execute("select uid from files_image where vid='$vidid';");
	    $muid = $rm->fields['uid'];
